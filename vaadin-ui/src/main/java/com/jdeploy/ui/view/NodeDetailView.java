...
public class NodeDetailView extends VerticalLayout implements HasUrlParameter<String> {
    ...
    public NodeDetailView(TopologyApiClient apiClient, VaadinActionAuthorizationService authorizationService) {
    ...
    public void setParameter(com.vaadin.flow.router.BeforeEvent event, @WildcardParameter String parameter) {
        ...
        try {
            ...
            hostname.setValue(req.hostname());
            ipAddress.setValue(req.ipAddress());
            roles.setValue(String.join(",", req.roles()));
